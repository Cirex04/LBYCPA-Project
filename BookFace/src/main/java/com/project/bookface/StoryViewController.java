...
public class StoryViewController {
    ...
    public void addStory(ActionEvent actionEvent) throws IOException {
        ...
        String filename = "src/main/java/com/example/bookface/StoryDatabase.csv";
    ...
    private void updateDatabase(Queue<Story> storyQueue) throws IOException {
        String filename = "src/main/java/com/example/bookface/StoryDatabase.csv";
        FileWriter fstream = new FileWriter(filename, true);
        BufferedWriter writer = new BufferedWriter(fstream);
        System.out.println(storyQueue.size());
        ...
        while (!storyQueue.isEmpty()){
            writer.write(storyQueue.peek().getUid() + ",");
            writer.write(storyQueue.peek().getDisplayName() + ",");
            writer.write(storyQueue.peek().getImageFile());
            writer.newLine();
            storyQueue.remove();
        }
    ...
    private void uploadImage(File source, String filename) throws IOException {
        File dest = new File("D:\\IntelliJ IDEA 2022.3.2\\Projects\\BookFace\\src\\main\\resources");
        InputStream is = null;
